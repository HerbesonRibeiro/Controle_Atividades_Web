...
#         where_clauses.append("a.descricao LIKE LOWER %s")
...
#     query_stats_mes = "SELECT COUNT(*) AS total FROM atividades WHERE colaborador_id = %s AND DATE_FORMAT('month', data_atendimento) = DATE_TRUNC('month', CURRENT_DATE)"
#     stats_mes = db.execute_query(query_stats_mes, (colaborador_id,), fetch='one')['total']
#     query_stats_semana = "SELECT COUNT(*) AS total FROM atividades WHERE colaborador_id = %s AND DATE_FORMAT('week', data_atendimento) = DATE_TRUNC('week', CURRENT_DATE)"
